    ...
    if (garmentUrl) {
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            uni.navigateTo({
              url: `/subpackages/resources/ResourcesSelection/ResourcesSelection?type=${type}&source=UploadWhole`
            });
          }
        }
      });
    ...
    uploadFrom(sourceType, type) {
      uni.chooseImage({
        ...
        success: (res) => {
          ...
          uni.navigateTo({
            url: `/subpackages/confirm/ConfirmCloth/ConfirmCloth?imageUrl=${encodeURIComponent(tempFilePath)}&type=${type}`
          });
        },
        fail: (err) => {
